...
public:
	...
	HRESULT init(const char* fileName, int width, int height, int maxFrameX, int maxFrameY, BOOL isTrans = FALSE, COLORREF transColor = RGB(0, 0, 0), float scale = 1.0f);
	...
	virtual void rotateRender(HDC hdc, float destX, float destY, int frameX, int frameY, float angle, float scale = 1.0f) override;
	...
	virtual void render(HDC hdc, float destX, float destY, int sourX, int sourY, int sourWidth, int sourHeight) override;
