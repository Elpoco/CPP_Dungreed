...
public:
	...
	ImageGp* addImage(HDC hdc, string strKey, string fileName, float scaleW = 1, float scaleH = 1);
	ImageGp* addFrameImage(HDC hdc, string strKey, string fileName, int maxFrameX, int maxFrameY);

	void render(string strKey, HDC hdc, float x, float y, int angle = 0, POINT rotateCenter = { 0,0 });
	void frameRender(string strKey, HDC hdc, float x, float y, int frameX, int frameY, int angle = 0, POINT rotateCenter = { 0,0 });
